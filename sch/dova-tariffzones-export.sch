...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2"
    ...
    <pattern id="CompositeFrame-frames">
        <rule context="ntx:CompositeFrame[ntx:TypeOfFrameRef/@ref='NL:BISON:TypeOfFrame:NL_DOVA_LISTS']/ntx:frames">
            <assert test=".[count(ntx:GeneralFrame/ntx:TypeOfFrameRef[@ref='NL:BISON:TypeOfFrame:NL_TariffZoneList'])=1]">Er moet exact 1 GeneralFrame (van type 'NL:BISON:TypeOfFrame:NL_TariffZoneList') worden geleverd als onderdeel van de BISON enumerations export</assert>
        </rule>
    </pattern>

    <pattern id="TariffZone">
        <rule context="ntx:GeneralFrame[ntx:TypeOfFrameRef/@ref='NL:BISON:TypeOfFrame:NL_TariffZoneList']">
            <assert test="ntx:Description">Description is verplicht</assert>
            <assert test="ntx:members">members is verplicht</assert>
            <assert test="ntx:members/ntx:TariffZone">members/TariffZone is verplicht</assert>
        </rule>
        <rule context="ntx:GeneralFrame[ntx:TypeOfFrameRef/@ref='NL:BISON:TypeOfFrame:NL_TariffZoneList']/ntx:members/ntx:TariffZone">
