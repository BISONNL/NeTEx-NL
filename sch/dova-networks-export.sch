...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2"
    ...
    <pattern id="CompositeFrame-frames">
        <rule context="ntx:CompositeFrame[ntx:TypeOfFrameRef/@ref='NL:BISON:TypeOfFrame:NL_DOVA_LISTS']/ntx:frames">
            <assert test=".[count(ntx:GeneralFrame/ntx:TypeOfFrameRef[@ref='NL:BISON:TypeOfFrame:NL_NetworkList'])=1]">Er moet exact 1 GeneralFrame (van type 'NL:BISON:TypeOfFrame:NL_NetworkList') worden geleverd als onderdeel van de BISON enumerations export</assert>
        </rule>
    </pattern>

    <pattern id="Network">
        <rule context="ntx:GeneralFrame[ntx:TypeOfFrameRef/@ref='NL:BISON:TypeOfFrame:NL_NetworkList']">
            <assert test="ntx:Description">Description is verplicht</assert>
            <assert test="ntx:members">members is verplicht</assert>
            <assert test="ntx:members/ntx:Network">members/Network is verplicht</assert>
            <assert test="ntx:members/ntx:TransportAdministrativeZone">members/TransportAdministrativeZone is verplicht</assert>
            <assert test="count(ntx:members/ntx:TransportAdministrativeZone)=count(ntx:members/ntx:TransportAdministrativeZone)">het aantal Network elementen binnen members moet gelijk zijn aan het aantal TransportAdministrativeZone elementen binnen members</assert>
        </rule>
        <rule context="ntx:GeneralFrame[ntx:TypeOfFrameRef/@ref='NL:BISON:TypeOfFrame:NL_NetworkList']/ntx:members/ntx:Network">
            <assert test="ntx:Name">Name is verplicht</assert>
            <assert test="ntx:ShortName">ShortName is verplicht</assert>
            <assert test="ntx:GroupOfLinesType">GroupOfLinesType is verplicht</assert>
            <assert test="ntx:AuthorityRef">AuthorityRef is verplicht</assert>
            <assert test="@version='any'">Attribuut version moet de waarde 'any' hebben</assert>
        </rule>
        <rule context="ntx:GeneralFrame[ntx:TypeOfFrameRef/@ref='NL:BISON:TypeOfFrame:NL_NetworkList']/ntx:members/ntx:TransportAdministrativeZone">
            <assert test="ntx:Name">Name is verplicht</assert>
            <assert test="ntx:ShortName">ShortName is verplicht</assert>
            <assert test="@version='any'">Attribuut version moet de waarde 'any' hebben</assert>
        </rule>
