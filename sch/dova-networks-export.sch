...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2"
    ...
    <sch:pattern id="CompositeFrame-frames">
        <sch:rule context="ntx:CompositeFrame[ntx:TypeOfFrameRef/@ref='NL:BISON:TypeOfFrame:NL_DOVA_LISTS']/ntx:frames">
            <sch:assert test=".[count(ntx:GeneralFrame/ntx:TypeOfFrameRef[@ref='NL:BISON:TypeOfFrame:NL_NetworkList'])=1]">Er moet exact 1 GeneralFrame (van type 'NL:BISON:TypeOfFrame:NL_NetworkList') worden geleverd als onderdeel van de BISON enumerations export</sch:assert>
        </sch:rule>
    </sch:pattern>

    <sch:pattern id="GeneralFrame">
        <sch:rule context="ntx:GeneralFrame[ntx:TypeOfFrameRef/@ref='NL:BISON:TypeOfFrame:NL_NetworkList']">
            ...
            <sch:assert test="ntx:members/ntx:Network">members/Network is verplicht</sch:assert>
            <sch:assert test="ntx:members/ntx:TransportAdministrativeZone">members/TransportAdministrativeZone is verplicht</sch:assert>
            <sch:assert test="count(ntx:members/ntx:Network)=count(ntx:members/ntx:TransportAdministrativeZone)">het aantal Network elementen binnen members moet gelijk zijn aan het aantal TransportAdministrativeZone elementen binnen members</sch:assert>
        </sch:rule>
    </sch:pattern>

    <sch:include href="PublicationDelivery/CompositeFrame/GeneralFrame/Network.sch"/>
    <sch:include href="PublicationDelivery/CompositeFrame/GeneralFrame/TransportAdministrativeZone.sch"/>
